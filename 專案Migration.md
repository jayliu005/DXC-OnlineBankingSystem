...
本文件整合以下三份文件的決策與執行內容，作為 `DXC-OnlineBankingSystem` 新專案開發的主要依據：
...
每次進行 `DXC-OnlineBankingSystem` 的分析、設計、程式修改、資料庫變更、建置、測試或部署前，Codex 必須先完整閱讀本文件，再開始操作。
...
- B 專案：`DXC-OnlineBankingSystem`，所有新程式碼只寫在此專案。
- 不直接在舊 JSF、Spring 4、Spring Web Flow 與 XML 架構中逐檔改寫。
- 舊專案在 Migration 期間保持可執行，以便人工比對新舊行為。
...
- 建立全新的空白 Oracle schema `DXCONLINEBANKING`。
...
1. 保留並凍結舊專案。
2. 建立全新的 `DXC-OnlineBankingSystem`。
3. 每次只分析及實作一項功能。
...
└── DXC-OnlineBankingSystem/    B：新專案，實際開發
```
...
```bash
codex -C /path/to/DXC-OnlineBankingSystem \
...
B 專案：/path/to/DXC-OnlineBankingSystem
- 所有新程式只能寫在此目錄
...
```bash
mkdir DXC-OnlineBankingSystem
cd DXC-OnlineBankingSystem
git init
...
│   ├── exception/
│   └── response/
└── DxcOnlineBankingApplication.java
```
